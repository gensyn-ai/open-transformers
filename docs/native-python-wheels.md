...
`scripts/audit_kit/probe_native_replay.py` emits deterministic input, init,
logit, loss, gradient, weight, optimizer and state hashes over two small QAT
steps. Run it in separate source/native environments and compare those fields.
It is a composition test, not a published training-interval replay. Installed
native validation must assert extension imports and avoid the test suite's
source-path injection in `tests/conftest.py`.
