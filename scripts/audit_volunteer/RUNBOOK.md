...
## Publishing a kit (maintainers)

`scripts/audit_kit/publish_audit_kit.sh` builds the pretrain wheel at HEAD,
stages the matching repop wheels, computes sha256s into `kit.json`, and
publishes wheel + manifest + `trajectory.json` immutably (`--no-clobber`) to
two prefixes under the same kit id:

- `gs://gensyn-audit-artifacts/audit-kit/<kit-id>/`, the provenance copy. The
  bucket carries no lifecycle expiry, so a kit URL in an audit
  receipt stays resolvable. It is closed, and no verifier can read it.
- `gs://gensyn-audit-public/audit-kit/<kit-id>/`, the copy this runbook cites.
  World-readable, holding only promoted kits.

The manifest records the exact `torch_requirement` from the repop wheels;
publishing refuses missing, non-exact, or conflicting pins. Promotion copies
from the immutable provenance prefix, never from a fresh wheel build. The
publisher validates the provenance digests, then reads every public file back
over credential-free https and compares it with provenance before reporting
success. A conflicting partial publish fails rather than being overwritten.

To defer promotion, publish with `--public-bucket ""`. The manifest still names
the default public URL. Later, from the same pretrain commit, run:

```bash
scripts/audit_kit/publish_audit_kit.sh --repop-commit <full-node-sha> --promote-only
```

This reuses the stored kit without rebuilding or needing a trajectory file.
The public bucket must match the URL recorded in the immutable manifest;
changing that URL requires a new kit id.
