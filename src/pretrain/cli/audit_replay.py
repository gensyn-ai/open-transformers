...
def _parse_args():
    ...
    p.add_argument(
        ...
        "(logs/metrics.jsonl) bit-for-bit; compare with "
        "scripts/repro/compare_rank0_losses.py. Also returned in the result "
...
def audit_replay(
    ...
    *,
    ...
    verifier can run against the run's logged losses (see
    scripts/repro/compare_rank0_losses.py) before accepting the audit's
    hand-off checkpoint. The records are also returned in the result dict as
